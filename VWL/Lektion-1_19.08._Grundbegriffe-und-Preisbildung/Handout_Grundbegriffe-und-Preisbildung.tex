...
Einkommensänderungen. Ausführliche Rechenbeispiele mit Schritt-für-Schritt-Lösungsweg
finden Sie im Abschnitt „Formeln \& Rechenbeispiele`` am Ende dieses Handouts.
...
\subsection{Direkte Preiselastizität der Nachfrage}

\begin{merksatz}[title={\bfseries Formel}]
\[
\varepsilon \;=\; \frac{\text{prozentuale Mengenänderung der Nachfrage}}{\text{prozentuale Preisänderung}}
\]
$|\varepsilon| > 1$: elastisch \textperiodcentered{} $|\varepsilon| < 1$: unelastisch
\textperiodcentered{} $|\varepsilon| = 0$: vollkommen unelastisch.
\end{merksatz}
...
\begin{enumerate}
  \item \textbf{Preisänderung berechnen:} $0{,}40~\text{€} \div 2{,}00~\text{€} = +20\,\%$.
  \item \textbf{Mengenänderung berechnen:} $-20 \div 1.000 = -2\,\%$.
  \item \textbf{Einsetzen:} $\varepsilon = -2\,\% \div (+20\,\%) = -0{,}1$, also $|\varepsilon| = 0{,}1$.
\end{enumerate}
\textbf{Interpretation:} $|\varepsilon| < 1$ --- die Nachfrage reagiert \emph{unelastisch}:
Die Käufer können auf das lebensnotwendige Gut kaum verzichten. Der Umsatz der Anbieter
\emph{steigt} bei der Preiserhöhung, weil der Mengenrückgang ($-2\,\%$) viel schwächer
ausfällt als die Preiserhöhung ($+20\,\%$). Zum Vergleich: Bei einem Modeartikel mit
Preis $+10\,\%$ und Menge $-25\,\%$ wäre $|\varepsilon| = 2{,}5$ --- \emph{elastisch},
der Umsatz würde sinken.
\end{beispiel}

\subsection{Kreuzpreiselastizität}

\begin{merksatz}[title={\bfseries Formel}]
\[
\varepsilon_K \;=\; \frac{\text{prozentuale Mengenänderung bei Gut A}}{\text{prozentuale Preisänderung bei Gut B}}
\]
Vorzeichen \textbf{positiv} $\Rightarrow$ Substitutionsgüter \textperiodcentered{}
Vorzeichen \textbf{negativ} $\Rightarrow$ komplementäre Güter.
\end{merksatz}

\begin{beispiel}
Der Butterpreis (Gut~B) steigt um $10\,\%$; die nachgefragte Menge an Margarine (Gut~A)
steigt um $5\,\%$.
\begin{enumerate}
  \item \textbf{Mengenänderung bei Gut A:} $+5\,\%$ (Margarine).
  \item \textbf{Preisänderung bei Gut B:} $+10\,\%$ (Butter).
  \item \textbf{Einsetzen:} $\varepsilon_K = +5\,\% \div (+10\,\%) = +0{,}5$.
\end{enumerate}
\textbf{Interpretation:} Das \emph{positive} Vorzeichen zeigt Substitute --- wird Butter
teurer, weichen die Käufer auf Margarine aus. Bei Komplementen (z.\,B. Drucker und
Patronen) wäre $\varepsilon_K$ \emph{negativ}: Ein teurerer Drucker senkt auch die
Patronen-Nachfrage.
\end{beispiel}
...
(Abschnitt „Formeln \& Rechenbeispiele``).
